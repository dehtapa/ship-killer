...
class Window:
  ...
  def init_buttons(self):
      self.button1 = Button(self.root, text="Create ships automatically", command=self.master.auto_build_ships)
      self.button1.grid(row=9, column=20)
      self.button2 = Button(self.root, text="Clear ships", command=self.master.clear_ships)
...
class Table():
  ...
  def draw_table_buttons(self):
    for x in range(1,self.rows +1):
      ...
      for y in range(1,self.rows+1):
        self.buttons[x][y] = Button(master=self.root, bg="azure2", command=lambda row=x, column=y: self.button_fired(row,column))
    ...
    for shipNumber in range(1, self.master.ships + 1):
      while True:
        ...
        if self.check_fields(neighbours, shipNumber, False) is True:
          ...
          if len(cleanedSideNeighbours) >= 2: break
      ...
      for unit in range(1, shipNumber ):
        while True:
          ...
          if self.check_fields(neighbours, shipNumber, False) is True:
            ...
            if len(newCleanedSideNeighbours) >= 1: break
  ...
  def get_side_neighbours(self, x, y):
    fields = []
    if x > 1: fields.append([x-1,y])
    if y > 1: fields.append([x,y-1])
    if x < self.rows - 1: fields.append([x+1,y])
    if y < self.rows - 1: fields.append([x,y+1])
    return fields
  ...
  def check_fields(self, fieldList, shipNumber, pull):
    ...
    for field in fieldList:
      ...
      if self.coords.matrix[x][y] > 0 and self.coords.matrix[x][y] is not shipNumber:
        if pull is False: return False
        fieldList.remove([x,y])
      if pull is False: return True
